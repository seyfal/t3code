 ...
 * `~/.prime/agent/sessions/**\/*.jsonl`) rather than relying on T3 Code's own
 ...
 *
 * v5 added `grok` and v6 adds `prime` to {@link UsageProviderKind}; v4
 * Claude/Codex buckets remain valid, so mixed-version environments keep
 * those totals instead of treating every older server as stale.
 */
export const USAGE_MERGE_COMPATIBLE_SINCE = 4 as const;

export const UsageProviderKind = Schema.Literals(["claude", "codex", "grok", "prime"]);
