...
// ACP does not expose PrimeAgent's private `streaming_reasoning` phase. Once it has
...
const DEFAULT_PRIME_AGENT_TURN_INACTIVITY_TIMEOUT_MS = 10 * 60 * 1_000;
...
const DEFAULT_PRIME_AGENT_ACTIVE_TOOL_INACTIVITY_TIMEOUT_MS = 30 * 60 * 1_000;
...
interface PrimeAgentTurnLivenessSignal {
...
interface PrimeAgentSessionContext {
  ...
  readonly livenessSignals: Queue.Queue<PrimeAgentTurnLivenessSignal>;
...
/** Detect PrimeAgent's enter_plan_mode tool call from ACP tool state. */
export function isPrimeAgentEnterPlanModeToolCall(toolCall: {
...
export function nextPrimeAgentPlanModeActive(
  ...
  if (!isPrimeAgentEnterPlanModeToolCall(toolCall)) {
...
const resolveSessionCallbackTurnId = (
  sessions: ReadonlyMap<ThreadId, PrimeAgentSessionContext>,
  threadId: ThreadId,
): TurnId | undefined => {
...
function parsePrimeAgentResume(raw: unknown): { sessionId: string } | undefined {
...
export function selectPrimeAgentPermissionOptionId(
  ...
  // PrimeAgent 4.6 often omits allow_always. T3 still offers "Always allow this session".
...
function selectAutoApprovedPermissionOption(
  ...
  return (
    selectPrimeAgentPermissionOptionId(request, "acceptForSession") ??
    selectPrimeAgentPermissionOptionId(request, "accept")
  );
}
  ...
  return Effect.gen(function* () {
    ...
    const turnInactivityTimeoutMs =
      ...
      Number.isFinite(requestedTurnInactivityTimeoutMs)
        ...
        : DEFAULT_PRIME_AGENT_TURN_INACTIVITY_TIMEOUT_MS;
    ...
    const activeToolInactivityTimeoutMs =
      ...
      Number.isFinite(requestedActiveToolInactivityTimeoutMs)
        ...
        : DEFAULT_PRIME_AGENT_ACTIVE_TOOL_INACTIVITY_TIMEOUT_MS;
      ...
      Effect.mapError(
        (cause) =>
          new ProviderAdapterRequestError({
            ...
            detail: "Failed to generate PrimeAgent runtime identifier.",
            cause,
          }),
      ),
    );
        ...
        Effect.mapError(
          (cause) =>
            new EffectAcpErrors.AcpTransportError({
              detail: "Failed to process PrimeAgent ACP callback.",
              cause,
            }),
        ),
      );
    ...
    const signalTurnLiveness = (ctx: PrimeAgentSessionContext, turnId: TurnId) =>
      Queue.offer(ctx.livenessSignals, { turnId }).pipe(Effect.asVoid);

    const beginTurnLiveness = (ctx: PrimeAgentSessionContext, turnId: TurnId) =>
      Effect.sync(() => {
        ctx.livenessTurnId = turnId;
        // Do not start a deadline until ACP has made observable progress.
        // PrimeAgent's private reasoning phase is not present in the ACP stream.
        ctx.lastTurnActivityAtNanos = undefined;
        ctx.activeToolCallIds.clear();
      });

    const clearTurnLiveness = (ctx: PrimeAgentSessionContext) => {
    ...
    const recordTurnActivity = Effect.fn("PrimeAgentAdapter.recordTurnActivity")(function* (
      ctx: PrimeAgentSessionContext,
      turnId: TurnId,
      event: Extract<
    ...
    const hasLivenessPause = (ctx: PrimeAgentSessionContext) =>
      ctx.pendingApprovals.size > 0 ||
      ctx.pendingUserInputs.size > 0 ||
      ctx.livenessUpdatesInFlight > 0;

    const livenessTimeoutFor = (ctx: PrimeAgentSessionContext) =>
    ...
    const isLiveTurn = (ctx: PrimeAgentSessionContext, turnId: TurnId) =>
    ...
    const settleStalledTurn = Effect.fn("PrimeAgentAdapter.settleStalledTurn")(function* (
      ctx: PrimeAgentSessionContext,
      turnId: TurnId,
    ) {
      return yield* withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          yield* settlePromptInFlight(ctx.threadId, turnId, ctx.acpSessionId, {
            errorMessage: `PrimeAgent ACP turn stalled without content or tool progress for ${livenessTimeoutFor(ctx).milliseconds}ms.`,
    ...
    const runTurnLivenessWatchdog = Effect.fn("PrimeAgentAdapter.runTurnLivenessWatchdog")(
      function* (ctx: PrimeAgentSessionContext) {
        while (true) {
          if (ctx.stopped) {
            return;
          }
    ...
    const logNative = (threadId: ThreadId, method: string, payload: unknown) =>
      Effect.gen(function* () {
        ...
        Effect.catchCause((cause) =>
          Effect.logWarning("Failed to write native PrimeAgent notification log.", {
            cause,
            threadId,
            method,
          }),
    ...
    const startSession: PrimeAgentAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const primeAgentModelSelection =
          ...
          const started = yield* Effect.gen(function* () {
            // Prime Agent's ACP surface has no client-directed extension
            // requests (no ask-user-question / exit-plan-mode analogues); its
            // extras arrive one-way inside `_meta` envelopes on session
            // updates and are safely ignored by this baseline adapter.
              ...
              mapAcpCallbackFailure(
                Effect.gen(function* () {
                  ...
                  if (input.runtimeMode === "full-access" || alreadyApproved) {
                    const autoApprovedOptionId =
                      input.runtimeMode === "full-access"
                        ...
                        : selectPrimeAgentPermissionOptionId(params, "accept");
                  ...
                  const selectedOptionId =
                    resolved === "cancel"
                      ? undefined
                      : selectPrimeAgentPermissionOptionId(params, resolved);
                  if (
          ...
          const requestedStartModelId = primeAgentModelSelection?.model
            ? resolvePrimeAgentAcpBaseModelId(primeAgentModelSelection.model)
            : undefined;
          const currentStartModelId = currentPrimeAgentModelIdFromSessionSetup(
            started.sessionSetupResult,
          );
          const currentStartReasoningEffort = currentPrimeAgentReasoningEffortFromSessionSetup(
            started.sessionSetupResult,
          );
          ...
          const boundModelId = yield* applyPrimeAgentAcpModelSelection({
            runtime: acp,
          ...
          const session: ProviderSession = {
            ...
            runtimeMode: input.runtimeMode,
            cwd,
            ...(boundModelId ? { model: resolvePrimeAgentAcpBaseModelId(boundModelId) } : {}),
            threadId: input.threadId,
            resumeCursor: {
          ...
          const ctx: PrimeAgentSessionContext = {
            ...
            promptsInFlight: 0,
            livenessSignals: yield* Queue.sliding<PrimeAgentTurnLivenessSignal>(1),
            ...
            currentReasoningEffort:
              requestedStartReasoningEffort !== undefined
                ? normalizePrimeAgentReasoningEffort(requestedStartReasoningEffort)
          ...
          const nf = yield* Stream.runDrain(
            ...
            Effect.catch((cause) =>
              Effect.logError("Failed to process PrimeAgent runtime notification.", { cause }),
            ),
          ...
          yield* offerRuntimeEvent({
            ...
            provider: PROVIDER,
            threadId: input.threadId,
            payload: { state: "ready", reason: "PrimeAgent ACP session ready" },
          });
          yield* offerRuntimeEvent({
            type: "thread.started",
            ...(yield* makeEventStamp()),
          ...
          Effect.gen(function* () {
            ...
            return yield* Effect.gen(function* () {
              ...
              const requestedTurnModelId = turnModelSelection?.model
                ? resolvePrimeAgentAcpBaseModelId(turnModelSelection.model)
                : undefined;
              ...
              // PrimeAgent ingests images only. Generic files reach the agent
              ...
              const currentModelId = yield* applyPrimeAgentAcpModelSelection({
                runtime: ctx.acp,
              ...
              if (requestedTurnReasoningEffort !== undefined) {
                ctx.currentReasoningEffort = normalizePrimeAgentReasoningEffort(
                  requestedTurnReasoningEffort,
                );
              }
              const displayModel = currentModelId
                ? resolvePrimeAgentAcpBaseModelId(currentModelId)
                : undefined;
              ...
              if (ctx.interruptedTurnIds.has(turnId)) {
                ...
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: "PrimeAgent prompt was interrupted during preparation.",
                });
              }
              if (steeringTurnId === undefined) {
              ...
              Effect.tapCause(() =>
                Effect.gen(function* () {
                  ...
                  yield* settlePromptInFlight(input.threadId, turnId, liveCtx.acpSessionId, {
                    errorMessage: "PrimeAgent prompt preparation failed.",
                    emitTurnCompletion: false,
                  });
                }),
              ),
            ...
            Effect.gen(function* () {
              ...
              if (ctx.acpSessionId !== prepared.acpSessionId) {
                yield* settlePromptInFlight(
                  ...
                  {
                    errorMessage: "PrimeAgent session changed before the turn completed.",
                    settleAllPrompts: true,
                  },
                );
                ...
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: "PrimeAgent session changed before the turn completed.",
                });
              }
            ...
            Effect.gen(function* () {
              ...
              yield* withThreadLock(
                ...
                settlePromptInFlight(input.threadId, prepared.turnId, prepared.acpSessionId, {
                  errorMessage: errorMessage ?? "PrimeAgent prompt request failed.",
                }),
              );
    ...
    const rollbackThread: PrimeAgentAdapterShape["rollbackThread"] = (threadId, numTurns) =>
      Effect.gen(function* () {
        ...
        return yield* new ProviderAdapterRequestError({
          ...
          detail: "PrimeAgent ACP sessions do not support provider-side rollback yet.",
        });
      });
