...
export const PrimeAgentDriver: ProviderDriver<PrimeAgentSettings, PrimeAgentDriverEnv> = {
  ...
  create: ({ instanceId, displayName, accentColor, environment, enabled, config }) =>
    Effect.gen(function* () {
      ...
      const checkProvider = checkPrimeAgentProviderStatus(effectiveConfig, processEnv, cwd).pipe(
      ...
      const snapshot = yield* makeManagedServerProvider<ProviderSnapshotSettings<PrimeAgentSettings>>({
        ...
        Effect.mapError(
          (cause) =>
            new ProviderDriverError({
              driver: DRIVER_KIND,
              instanceId,
              detail: `Failed to build PrimeAgent snapshot: ${cause.message ?? String(cause)}`,
              cause,
            }),
        ),
      );
