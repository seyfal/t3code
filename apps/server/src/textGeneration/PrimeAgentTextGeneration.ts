...
import { type PrimeAgentSettings, type ModelSelection } from "@t3tools/contracts";
...
export const makePrimeAgentTextGeneration = Effect.fn("makePrimeAgentTextGeneration")(function* (
  ...
  const runPrimeAgentJson = <S extends Schema.Top>({
    ...
    Effect.gen(function* () {
      const resolvedModel = resolvePrimeAgentAcpBaseModelId(modelSelection.model);
      ...
      const promptResult = yield* Effect.gen(function* () {
        ...
        yield* applyPrimeAgentAcpModelSelection({
          runtime,
          currentModelId: currentPrimeAgentModelIdFromSessionSetup(started.sessionSetupResult),
          currentReasoningEffort: currentPrimeAgentReasoningEffortFromSessionSetup(
          ...
          mapError: (cause) =>
            new TextGenerationError({
              operation,
              detail: "Failed to set PrimeAgent ACP base model for text generation.",
              cause,
            }),
        });
        ...
        Effect.flatMap(
          Option.match({
            onNone: () =>
              Effect.fail(
                new TextGenerationError({ operation, detail: "PrimeAgent ACP request timed out." }),
              ),
        ...
        Effect.mapError((cause: EffectAcpErrors.AcpError | TextGenerationError) =>
          isTextGenerationError(cause)
            ...
            : new TextGenerationError({
                operation,
                detail: "PrimeAgent ACP request failed.",
                cause,
              }),
        ),
      );
        ...
        return yield* new TextGenerationError({
          ...
          detail:
            promptResult.stopReason === "cancelled"
              ? "PrimeAgent ACP request was cancelled."
              : "PrimeAgent Agent returned empty output.",
        });
      }
        ...
        Effect.catchTags({
          SchemaError: (cause) =>
            Effect.fail(
              new TextGenerationError({
                operation,
                detail: "PrimeAgent Agent returned invalid structured output.",
                cause,
              }),
            ),
        }),
      ...
      Effect.mapError((cause) =>
        isTextGenerationError(cause)
          ...
          : new TextGenerationError({
              operation,
              detail: "PrimeAgent ACP text generation failed.",
              cause,
            }),
      ),
      Effect.scoped,
