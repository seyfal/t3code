...
import * as PrimeAgentSessionImport from "./provider/PrimeAgentSessionImport.ts";
...
const makeWsRpcLayer = (
  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      // Built inline rather than as a runtime layer: every dependency
      // (engine, projections, config, filesystem) is already in this scope.
      const primeAgentImport = yield* PrimeAgentSessionImport.make;
      ...
      return WsRpcGroup.of({
        ...
        [WS_METHODS.serverListPrimeAgentImports]: (_input) =>
          observeRpcEffect(
            WS_METHODS.serverListPrimeAgentImports,
            primeAgentImport.listCandidates(),
            {
              "rpc.aggregate": "server",
            },
          ),
        [WS_METHODS.serverImportPrimeAgentSession]: (input) =>
          observeRpcEffect(
            WS_METHODS.serverImportPrimeAgentSession,
            primeAgentImport.importSession(input),
            {
              "rpc.aggregate": "server",
            },
          ),
